...
protocol JDP_ShowListView_delegate:class {
    /// 总共有几个View
...
///  用来展示横向等宽的界面
class JDP_ShowListView: UIView {
    /// 单个数字的View
    weak var delegate:JDP_ShowListView_delegate!
    ...
    private func initializeConstraint() {
        ...
        numberVs.first!.leftAnchor.constraint(equalTo: self.leftAnchor).isActive = true
        numberVs.last!.rightAnchor.constraint(equalTo: self.rightAnchor).isActive = true
        for i in 0 ..< numberVs.count {
            numberVs[i].topAnchor.constraint(equalTo: self.topAnchor).isActive = true
            numberVs[i].bottomAnchor.constraint(equalTo: self.bottomAnchor).isActive = true
